...
int main( int argc, char** argv )
{
  ...
  typedef functors::BallConstantPointFunction<Point,float> KernelFunction;
  ...
  for ( Domain::ConstIterator it = dvcm.domain().begin(), itE = dvcm.domain().end();
        it != itE; ++it )
    {
	  ...
	  double boundedLambda2 = sqrt(3) * (pow(rmax, 2) - 1) / 2.;
	  double lambda0 = (eval[0] < 1) ? 1 : eval[0];
	  double lambda1 = (eval[1] < 1) ? 1 : eval[1];
	  double lambda2 = (eval[2] < 1) ? 1 : eval[2];
	  double ellipseArea = (4./3) * M_PI * sqrt(lambda0 * lambda1 * lambda2);
	  double ellipseAreaThMin = (4./3) * M_PI * std::min(R, (float) boundedLambda2) * r * 1.0;
	  double ellipseAreaThMax = (4./3) * M_PI * std::min(R, (float) boundedLambda2) * r * r;

      if ( ellipseArea > ellipseAreaThMin && ellipseArea < ellipseAreaThMax
        )
        {
